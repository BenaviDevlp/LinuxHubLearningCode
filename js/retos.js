...
function renderMisionesTerminal() {
  ...
  cont.innerHTML = `
    ...
    <div class="mis-obj">
      <span class="mis-check">${icono("circle")}</span>
      <div>
...
function verificarMisionTerminal() {
  ...
  if (check) { check.innerHTML = icono("check-circle", "hecha"); }
...
function renderizarProyectoModulo(modId) {
  ...
  cont.innerHTML = `
    <div class="proy-cab">
      ...
      <h3>${escapar(proy.titulo)} ${hecho ? '<span class="proy-ok">' + icono("check") + ' completado</span>' : ""}</h3>
    </div>
