...
// Devuelve el markup de un icono SVG del sprite (heredan color con currentColor)
function icono(id, clase) {
  return `<svg class="ic ${clase || ""}" aria-hidden="true"><use href="#ic-${id}"></use></svg>`;
}
// Mapea un módulo de la Academia a su icono del sprite
function iconoModulo(m) {
  const mapa = {
    "linux-principiante": "sprout", "linux-intermedio": "gear", "linux-avanzado": "rocket",
    "bash": "scroll", "python": "code", "hacking": "shield", "ingles": "globe", "redes": "network"
  };
  return icono(mapa[m.id] || "book-open");
}
// Traduce un emoji conocido a su icono SVG (para datos que guardan emojis, p.ej. logros)
const EMOJI_ICONOS = {
  "📝":"file-pen","✍️":"pen-line","✍":"pen-line","🏷️":"tag","🏷":"tag","🧭":"compass",
  "💻":"terminal","🎯":"target","🌟":"sparkles","🏅":"medal","📖":"book-open","✅":"check-circle",
  "📚":"library","🎓":"cap","📜":"award","🐍":"code","⚖️":"scale","⚖":"scale","🛡️":"shield","🛡":"shield",
  "🌐":"globe","🔥":"flame","⭐":"star","🧰":"wrench","🧪":"flask","📅":"calendar","🏗️":"hammer","🏗":"hammer","🔒":"lock"
};
function emojiAIcono(e, clase) {
  const id = EMOJI_ICONOS[e];
  return id ? icono(id, clase) : escapar(e);
}
...
function aplicarTema(tema) {
  ...
  if (btn) btn.innerHTML = icono(tema === "oscuro" ? "moon" : "sun");
}
...
function renderizarHistorialQuiz() {
  ...
  let html = `<h4>${icono("bar-chart")} Intentos anteriores</h4>`;
...
function iniciarQuiz() {
  ...
  html += `<button class="btn-primario" onclick="corregirQuiz()">${icono("check")} Corregir test</button>`;
...
function mostrarResultadoQuiz(aciertos, total, temasFallados) {
  ...
  let repasoHtml = "";
  if (repasar.length === 0) {
    repasoHtml = `<p style="color:var(--verde-mint);margin-top:14px;">${icono("sparkles")} ¡Perfecto! No necesitas repasar nada.</p>`;
  } else {
    repasoHtml = `<div class="quiz-repaso"><h4>${icono("book-open")} Te conviene repasar:</h4><ul>${
  ...
  cont.innerHTML = `
    <div class="quiz-resultado-caja">
      ...
      <div style="margin-top:18px;display:flex;gap:10px;justify-content:center;flex-wrap:wrap;">
        <button class="btn-primario" onclick="iniciarQuiz()">${icono("rotate")} Reintentar</button>
...
function renderizarLogros() {
  ...
  html += `<div class="panel-logros-grid panel-logros">` + LOGROS.map(l => {
    ...
    return `<div class="logro ${ok ? "desbloqueado" : ""}">
              <span class="icono">${ok ? emojiAIcono(l.icono) : icono("lock")}</span>
  ...
  const html = MODULOS.map(m => {
    ...
    return `
      <div class="tarjeta-modulo" onclick="abrirModulo('${m.id}')">
        <span class="icono" aria-hidden="true">${iconoModulo(m)}</span>
        <h3>${escapar(m.nombre)} ${completo ? icono("check-circle", "hecha") : ""}</h3>
...
function abrirModulo(modId) {
  ...
  document.getElementById("moduloTitulo").innerHTML = iconoModulo(modulo) + " " + escapar(modulo.nombre);
...
function renderizarListaLecciones() {
  ...
  document.getElementById("listaLecciones").innerHTML = lecciones.map((l, i) => `
    <button class="item-leccion ${i === leccionActual ? "activa" : ""}" onclick="renderizarLeccion(${i})">
      <span class="estado">${hechas[l.id] ? icono("check-circle", "hecha") : icono("circle")}</span>
...
function renderizarLeccion(i) {
  ...
  html += bloque(icono("book-open"), "Concepto", `<p>${conCodigo(l.concepto)}</p>`);
  if (l.ejemplos) html += bloque(icono("wrench"), "Ejemplos prácticos y casos de uso", `<p>${conCodigo(l.ejemplos)}</p>`);
  if (l.codigo)   html += bloque(icono("code"), "Código de ejemplo comentado", `<pre class="bloque-codigo"><code>${escapar(l.codigo)}</code></pre>`);
  const errs = l.errores || l.erroresComunes;        // compatibilidad de nombres
  if (errs)       html += bloque(icono("alert"), "Errores comunes", lista(errs));
  if (l.tips)     html += bloque(icono("cap"), "Tips del profesor", lista(l.tips));
  if (l.defensa)  html += bloque(icono("shield-check"), "Contraparte defensiva (blue team)", `<p>${conCodigo(l.defensa)}</p>`);
  ...
  if (l.comandos && l.comandos.length) {
    const chips = l.comandos.map(id => {
      ...
      return c ? `<button class="chip-comando" onclick="irAComando('${id}')">${icono("wrench")} ${escapar(c.nombre)}</button>` : "";
    }).join("");
    if (chips) html += bloque(icono("link"), "Comandos relacionados (abre su ficha)", `<div class="chips-comandos">${chips}</div>`);
  }
  if (l.resumen)  html += bloque(icono("file-text"), "Mini resumen", `<p>${conCodigo(l.resumen)}</p>`);
  if (l.preguntas) html += bloque(icono("help"), "Preguntas de repaso", lista(l.preguntas));
  ...
  document.getElementById("listaCertificados").innerHTML = MODULOS.map(m => {
    ...
    return `
      <div class="cert-card ${completo ? "desbloqueado" : ""}">
        <span class="cert-icono">${completo ? icono("award") : icono("lock")}</span>
        <div class="cert-info">
...
function generarSVGCertificado(nombre, titulo, fecha) {
  ...
  <svg xmlns="http://www.w3.org/2000/svg" width="1000" height="680" viewBox="0 0 1000 680" font-family="Segoe UI, Ubuntu, sans-serif">
    ...
    <path d="M476 84 L496 104 L476 124" fill="none" stroke="#6fbf4f" stroke-width="9" stroke-linecap="round" stroke-linejoin="round"/>
    <rect x="502" y="116" width="30" height="9" rx="4" fill="#6fbf4f"/>
...
function renderizarFiltrosComandos() {
  ...
  document.getElementById("filtrosComandos").innerHTML = cats.map(c => {
    const etiqueta = (c === "★ Favoritos") ? icono("star") + " Favoritos" : escapar(c);
    return `<span class="chip-etiqueta ${categoriaCmd === c ? "activa" : ""}" onclick="filtrarCategoriaCmd('${c}')">${etiqueta}</span>`;
  }).join("");
}
...
function renderizarComandos() {
  ...
  cont.innerHTML = lista.map(c => {
    ...
    const aviso = c.aviso
      ? `<div class="cmd-aviso">${icono("alert")} ${escapar(c.aviso)}</div>` : "";
    return `
      <article class="cmd-ficha" id="cmd-${c.id}">
        <div class="cmd-cabecera">
          ...
          <button class="cmd-fav ${esFav ? "activo" : ""}" onclick="toggleFavComando('${c.id}')" title="Favorito" aria-label="Marcar como favorito">${esFav ? icono("star", "relleno") : icono("star")}</button>
        </div>
        ...
        ${c.caso ? `<div class="cmd-caso"><b>${icono("briefcase")} En un trabajo real:</b> ${escapar(c.caso)}</div>` : ""}
        ${aviso}
        <div class="cmd-fuente">${icono("file")} Fuente: ${escapar(c.fuente)}</div>
      </article>`;
  }).join("");
}
